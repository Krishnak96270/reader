...
// Function to request a screen wake lock to prevent screen from turning off during speech
...
function updateStatus(msg) {
    ...
    // Update Media Session state for background controls & notifications
    if ('mediaSession' in navigator) {
        if (msg.includes('Playing')) {
            navigator.mediaSession.playbackState = 'playing';
        } else if (msg.includes('Paused')) {
            navigator.mediaSession.playbackState = 'paused';
        } else if (msg.includes('Stopped') || msg.includes('loaded')) {
...
fileInput.addEventListener('change', async (e) => {
    ...
    // Update media session title for the new file
    if ('mediaSession' in navigator) {
        navigator.mediaSession.metadata.title = file.name;
        navigator.mediaSession.metadata.artist = 'DocuReader';
    }
...
// --- Mutation Observer for Live Content Changes (e.g., Google Translate) ---
// This ensures that if the browser translates the content, we read the new text.
const observer = new MutationObserver(() => {
    // A brief debounce to handle rapid-fire mutations from translator scripts
    clearTimeout(window.contentChangeTimer);
    window.contentChangeTimer = setTimeout(() => {
        console.log('Content has changed, updating speech state.');
        
        // If speech is active, stop it, as the text source has changed.
        if (window.speechSynthesis.speaking || isPaused) {
            releaseWakeLock();
            isStopped = true;
            isPaused = false;
            window.speechSynthesis.resume(); // clear any pause state
            window.speechSynthesis.cancel();
            updateStatus('⏹ Text changed, playback stopped.');
        }

        // Re-calculate chunks and reset progress for the new content
        updateSpeechChunks();
        currentChunkIndex = 0;
        updateProgressUI();
    }, 250); // 250ms debounce
...
function speakNextChunk() {
    ...
    if (isStopped || currentChunkIndex >= speechChunks.length) {
        isStopped = true;
        isPaused = false; // Ensure pause state is cleared
        releaseWakeLock();
        return;
    }
    ...
    // Voice Selection Logic: Prioritize high-quality, network-based voices for clarity.
    if (availableVoices.length === 0) {
        availableVoices = window.speechSynthesis.getVoices();
    }
    if (availableVoices.length > 0) {
        const targetLang = pageLang.toLowerCase();
        ...
        const scoredVoices = availableVoices
            .map(voice => {
                ...
                if (voiceLang === targetLang) {
                    score += 4; // Perfect match (e.g., 'en-US')
                } else if (voiceLang.startsWith(targetLangPrefix)) {
                    score += 2; // Base language match (e.g., 'en')
                }

                // 2. Quality Heuristics (only apply if language matches)
                if (score > 0) {
                    if (!voice.localService) {
                        score += 4; // Heavily prefer network voices (higher quality)
                    }
                    if (voice.name.includes('Google')) {
                        score += 2; // Prefer Google-branded voices
                    }
                }
...
btnPlay.addEventListener('click', async () => {
    // Case 1: Browser pause worked correctly.
    ...
    // Case 2: Browser pause is buggy. Our flag is set.
    else if (isPaused) {
        isPaused = false;
        // We can't reliably resume, so restart speech from the current sentence.
        // speakContent will request its own wake lock.
        await speakContent(currentChunkIndex);
    } else if (!window.speechSynthesis.speaking || isStopped) {
        // Case 3: Not playing at all, or was stopped. Start fresh.
        await speakContent(0);
    }
});
...
btnPause.addEventListener('click', () => {
    if (window.speechSynthesis.speaking && !isPaused) {
        ...
        isPaused = true; // Track that pause was user-initiated
        updateStatus('⏸ Paused');
    }
});
...
async function skipSeconds(seconds) {
    ...
    await speakContent(targetIndex);
}
...
popupMenu.addEventListener('click', async () => {
    popupMenu.style.display = 'none';
